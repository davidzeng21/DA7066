...
def extract_northernmost_observation(data):
    """
    Extracts the northernmost observation of a butterfly species over time.

    Args:
        data (list): A list of dictionaries containing butterfly observation data.

    Returns:
        dict: A dictionary with years as keys and the northernmost latitude as values.
    """
    ...
    for row in data:
        try:  # try to convert the date to a year and the latitude to an integer
            ...
            continue
    return dict(sorted(year_latitude.items()))  # sort the dictionary by year
...
def plot_northernmost_observation(data):
    ...
    sorted_year_latitude = extract_northernmost_observation(data)
